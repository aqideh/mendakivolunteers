...
|---|---|---|---|---|
| Shared migration history | Open technical debt | The shared MakLom tables were bootstrapped into the KELUARGA Supabase projects during the database merger, but the three bootstrap migration files are not yet present in this repository. | Staging database contains the shared schema and current domain migrations. | Reconstruct/commit the shared baseline before production promotion so a clean local `supabase db reset` can reproduce the merged platform. |
...
| FormSG ingestion | Schema ready; connector pending | MakLom has a lead domain, but automated FormSG response ingestion still needs its production ingestion mechanism and field mapping. | FormSG remains the public volunteer CTA. MakLom supports lead records and deliberate conversion. | Implement/import the FormSG response feed with source response IDs and idempotency. |
