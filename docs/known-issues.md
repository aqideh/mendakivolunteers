...
|---|---|---|---|---|
| YM Hub production data | Integration dependency | Personal dashboard, official attendance, verified hours and points depend on reliable authoritative YM Hub snapshots. | Read models fail explicitly rather than fabricating records. | Complete controlled batch ingestion and reconciliation, then future API adapter if approved. |
| YM Hub batch centre | Planned | The repository has the projection/read-model foundation, but not a complete staff-operated import centre for all agreed Salesforce report types. | Manual/integration work remains outside the volunteer-facing UI. | Build versioned import templates, validation, checksums, previews, exception reports and history. |
...
| Separate authentication | Known constraint | KELUARGA and YM Hub use separate accounts/sessions. | Product copy must avoid implying shared sign-in. | Reassess SSO only as a separate approved identity project. |
| Pathway position | Planned feature | Pathway maps exist, but an individual volunteer cannot yet be assigned to a current pathway stage. | Volunteers can browse the published pathway tree. | Add staff-managed volunteer-stage assignment with stable stage keys and audit history. |
...
| Volunteer.gov.sg import | Transitional | Scheduled opportunity import exists but is not the intended long-term source once YM Hub/approved registration flows mature. | Current import remains isolated from authoritative attendance/points logic. | Retire or narrow it when the replacement source is confirmed. |
...
Future integration should map operational event records to the authoritative volunteer identity through controlled reconciliation.
...
- failed YM Hub imports must not produce substitute official records;
- stale imported data must be distinguishable from current data;
