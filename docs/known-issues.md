...
| Gamification rollout | Policy/integration dependency | Manual staff-recognition awards are now available, while attendance-derived awards still require approved rules plus verified YM Hub attendance. | Roster check-in cannot award points; manual recognition is separately identified and audited. | Approve attendance rule values/effective dates and a staff-use policy for manual recognition before broad rollout. |
