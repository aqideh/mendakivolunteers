...
Volunteer browsers do not receive direct `SELECT` access to `pathways.volunteer_positions`, because the history rows also contain staff reasons, notes and actor identifiers. Personal status is returned through the account-scoped `core.get_current_pathway_positions_snapshot()` security-definer function, which resolves the signed-in volunteer internally and emits only volunteer-safe position fields. Staff listing remains server-side and role-gated.
