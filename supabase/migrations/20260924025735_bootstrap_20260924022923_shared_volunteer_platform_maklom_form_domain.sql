...
create table if not exists public.form_submissions (
  id text primary key,
  batch_id text not null references public.form_import_batches(id) on delete restrict,
  submission_type text not null check (submission_type in ('sign_in','sign_out')),
  ...
  shirt_quantity integer not null default 0 check (shirt_quantity >= 0),
  shirt_size text,
  age_group text,
  gender text,
  volunteer_tenure text,
  volunteer_frequency text,
  briefing_rating smallint check (briefing_rating between 1 and 5),
  onboarding_rating smallint check (onboarding_rating between 1 and 5),
  role_satisfaction_rating smallint check (role_satisfaction_rating between 1 and 5),
  staff_support_rating smallint check (staff_support_rating between 1 and 5),
...
create table if not exists public.attendance_reconciliations (
  id text primary key,
  batch_id text not null references public.form_import_batches(id) on delete cascade,
  volunteer_id text references public.volunteers(id) on delete set null,
  sign_in_submission_id text references public.form_submissions(id) on delete restrict,
  sign_out_submission_id text references public.form_submissions(id) on delete restrict,
  event_name text not null,
  ...
  match_status text not null check (match_status in ('matched','missing_sign_in','missing_sign_out')),
...
alter table public.attendance_log add constraint attendance_log_form_reconciliation_id_fkey foreign key (form_reconciliation_id) references public.attendance_reconciliations(id) on delete set null;
...
grant select,insert,update,delete on public.form_import_batches,public.attendance_reconciliations to authenticated,service_role;
