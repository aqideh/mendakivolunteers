...
drop table if exists public.volunteer_lead_status_history cascade;
drop table if exists public.volunteer_leads cascade;
drop function if exists public.set_volunteer_lead_updated_fields();
drop function if exists public.track_volunteer_lead_status();
...
begin
  ...
  if v_lead.status='converted' and v_lead.converted_volunteer_id is not null then
    return jsonb_build_object(
      ...
      'keluarga_volunteer_id',v_lead.keluarga_volunteer_id
    );
  end if;
  ...
  if v_match_count=1 and v_existing_id is not null then
    ...
    set status='converted',
        ...
        keluarga_volunteer_id=v_profile.core_volunteer_id,
        converted_at=now()
    where id=v_lead.id;
...
comment on table public.volunteer_leads is
  'MakLom prospective-volunteer lead inbox. FormSG respondents remain leads until an authorised staff member accepts and converts them into the shared canonical volunteer identity.';
