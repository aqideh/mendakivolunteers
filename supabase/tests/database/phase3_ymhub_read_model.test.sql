...
select ok(
  ...
  'authenticated users can read registration snapshots through RLS'
);
...
select ok(
  ...
  'authenticated users can read attendance snapshots through RLS'
);
...
select throws_ok(
  ...
  'verified attendance requires verified hours and a verification timestamp'
);
...
select is(
  (
    ...
    where volunteer_id = '10000000-0000-4000-8000-000000000001'
      and state = 'verified'
  ),
  7.50::numeric(8, 2),
  'prototype verified hours total 7.50'
);
...
select is(
  ...
  2,
  'linked volunteer sees only their own registration snapshots'
);

select is(
  (select count(*)::integer from ymhub.attendance_snapshots),
  3,
  'linked volunteer sees only their own attendance snapshots'
);
...
select is(
  (select count(*)::integer from ymhub.registration_snapshots),
  0,
  'suspended volunteer cannot read downstream registration data'
);
